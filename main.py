...
class MultiLevelCacheSimulator:
    ...
    def access(self, address):
        ...
        if hit_l2:
            ...
            self.L2.access(address)  # Fill L2 on miss
        self.L1.access(address)  # Fill L1 on miss or L2 hit
    ...
    def stats(self):
        ...
        return hit_rate_l1, hit_rate_l2, amat
...
def run_experiments():
    ...
    with open("cache_performance_comparison.csv", "w", newline="") as f:
        ...
        # Plot AMAT for different workloads & policies, grouped by block size
    df = pd.DataFrame(results)

    plt.figure(figsize=(14, 8))  # Slightly taller for text box
    ...
    for workload in workloads:
        for policy in policies:
            ...
            # Aggregate by Block Size average AMAT (over configs)
    ...
    # Prepare summary text for the box
    ...
    # Add the summary text box to the figure (bottom right corner)
    ...
    # Summary analysis printed to terminal
    print("\n===== Analysis Summary =====")
    for workload in workloads:
        for policy in policies:
            subset = df[(df['Workload'] == workload) & (df['Policy'] == policy)]
    ...
    plt.tight_layout(rect=[0, 0.1, 1, 1])  # Leave space for the text box below
